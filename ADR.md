...
- Stores update metadata in `~/.config/devrunner/update.json`
- Shows changelog notification on next devrunner (respects --quiet flag)
...
**Decision**: 
- Order: hardcoded defaults → `~/.config/devrunner/config.toml` (global) → `./devrunner.toml` (local) → CLI args
...
**Decision**: 
- Bash: `~/.local/share/bash-completion/completions/devrunner` (user-local, no sudo)
- Zsh: requires fpath and compinit setup in `~/.zshrc`
- Fish: `~/.config/fish/completions/devrunner.fish` (with mkdir if needed)
...
- All tests devrunner via `cargo test` and `make precommit`
...
**Example**: `devrunner precommit` in a Rust project with Makefile correctly runs `make precommit` instead of failing with `cargo precommit`.
...
- Build commands: `nx <task>`, `turbo devrunner <task>`, `lerna devrunner <task>`
- Use `UnknownValidator` (trust the tool) - these tools handle command validation internally
**Example**: `devrunner build` in a project with `turbo.json` executes `turbo devrunner build` instead of `npm devrunner build`.
...
- Store last check timestamp in `~/.config/devrunner/last_update_check`
