---
feature: "devrunner - Universal Task Runner"
spec: |
...
- [x] 2.06 Add --dry-devrunner flag to show command without executing - Commit: e562098
...
- [x] 5.01 Implement global config parsing (~/.config/devrunner/config.toml) - Commit: e562098
- [x] 5.02 Implement local project config (./devrunner.toml) - Commit: e562098
...
- [x] 6.06 Store update metadata in ~/.config/devrunner/update.json - Commit: e562098
- [x] 6.07 Show update notification with changelog on next devrunner - Commit: e562098
...
- [x] 13.04 Build commands: nx <task>, turbo devrunner <task>, lerna devrunner <task>
...
- [ ] 15.02 Allow custom runners via .devrunner-plugins/ directory
- [ ] 15.03 Support user-defined aliases (devrunner t -> devrunner test)
...
- [ ] 16.01 Create VS Code extension for devrunner command palette
...
### Feature 20: Feature - Custom User Commands via devrunner.toml
...
- [ ] 20.02 Update `src/runner.rs` logic to prioritize `devrunner.toml` commands over auto-detection
- [ ] 20.03 Implement a `CustomDetector` that returns a runner for these user-defined commands
- [ ] 20.04 Support simple aliases (e.g., `t = "test"`) and full commands (e.g., `db:reset = "docker-compose devrunner web rake db:reset"`)
- [ ] 20.05 Add integration tests verifying `devrunner.toml` overrides standard detectors
